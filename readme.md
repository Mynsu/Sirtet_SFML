...
테트리스를 만든 이유는
----------------------
1. 심사원께 익숙한 게임 코드라면 **피로**가 조금이라도 덜할 것이고,   
2. 3D 그래픽스 API/셰이더, 쿼터니언/텐서/미적분 등이 없어 심사원께서 코드 **기본**만 보시기에 좋고,   
...
4. 제 자신이 **하고 싶지 않은 게임**이더라도 끝까지 만들어 낼 수 있을지 궁금했기 때문입니다.
...
3. 빌드된 Sirtet.exe와 Game.dll이 있는 디렉토리에 lua53.dll 압축 풀기   
+ SFML 2.5.0 이상   
예)   
...
3. 빌드된 Server.exe가 있는 디렉토리에 sfml-graphics-2.dll, sfml-system-2.dll, sfml-window-2.dll 압축 풀기(이 의존성은 없앨 예정입니다.)   
   빌드된 Sirtet.exe와 Game.dll이 있는 디렉토리에 openal32.dll, sfml-audio-2.dll, sfml-graphics-2.dll, sfml-system-2.dll, sfml-window-2.dll 압축 풀기   
+ 윈도우즈 7 이상입니다.


빌드하기 전에!
--------------
서버들의 IP 주소가 localhost로 설정되어 있습니다.  staticLibs/GameLib/Common.h에서 수정할 수 있습니다.


심사원께서 궁금하실 만한 것들을 정리했습니다.
---------------------------------------------
**#성능 우선** **#가독성** **#문체**   
보안을 최우선으로, 성능을 우선으로, 가독성을 그 다음으로 코딩했습니다.   
협업하는 동료는 물론 언젠가 다시 읽을 작성자를 괴롭히지 않으려면, 코드를 적고 읽기 어렵지 않은지 퇴고해야 합니다.

**#헤더 파일을 포함*include*하는 순서** **#간접 포함 없음**   
의존에 따라 C, 표준*standard*, 제 3자*3rd party*, 직접 만든 라이브러리, 나머지 헤더 파일 순으로 포함했습니다.   
예외로, WIN32_LEAN_AND_MEAN을 정의한 다음 포함하면 컴파일 오류가 발생하는 헤더 파일은 가장 먼저 포함했습니다.   
먼저 포함한 헤더 파일에 이미 포함된 헤더 파일이라도 생략하지 않고 명시해 포함했습니다.   
( 각 프로젝트의 pch.h 등 )

**#전방 선언*forward declaration***   
헤더 파일을 가능한 한 소스 파일에만 포함했습니다.   
헤더 파일에 선언된 포인터와 레퍼런스를 타입 체크해야 하더라도, 헤더 파일에 이 타입의 헤더 파일을 포함하지 않고 타입을 전방 선언하여 순환 참조를 예방했습니다.   
( dlls/Game/ui/NextTetriminoPanel.h와 PlayView.h, dlls/Game/scene/online/ 아래 헤더 파일들, exes/Server/Room.h )

**#상수를 모아둔 헤더 파일** **#모듈은 아직**   
공유하는 상수를 모아둔 헤더 파일은 있습니다만, 공유하지 않는 상수는 각 소스 파일에 있습니다.   
헤더 파일에서 어떤 상수를 수정하면 이 상수를 쓰지 않는 소스 파일마저 이 헤더 파일을 포함한다는 것만으로 다시 빌드하기 때문에, 단순히 상수라고 한 헤더 파일에 모아두어서는 안 됩니다.   
( staticLibs/GameLib/Common.h )

**#인터페이스** **#추상 클래스** **#순수 가상 함수**   
기반*base* 클래스, 직계존속(?)*parent* 클래스, 순수 가상 함수만 선언된 추상 클래스라는 이명들 대신 자바의 인터페이스라는 이름을 차용하겠습니다.  이 레퍼지토리에서 다른 의미의 '인터페이스'는 쓰지 않습니다.   
인터페이스 이름 앞에 알파벳 대문자 I를 붙여 다른 클래스와 구분합니다.  이로써 상속과 같은 문법이지만 중의적이지 않게 되고, 클래스 계층을 쉽게 파악할 수 있으며, 실수로 인스턴스를 만들지 않도록 돕습니다.   
또, 인터페이스 생성자의 접근 한정자를 protected로 두어 인스턴스를 만드는 실수를 조금이라도 예방했습니다.

**#상속**   
인터페이스 구현*implement*에만 상속을 썼습니다.   
다음과 같은 이유로 상속은 지양해야 합니다.   
1. 연필에도 볼펜에도 심이 있습니다.  그래서 심을 속성으로 갖는 펜 클래스를 두고 연필 클래스와 볼펜 클래스가 펜 클래스를 상속했습니다.  시간이 지나 심이 없는 스타일러스 펜도 다루게 되었습니다.  스타일러스 펜 클래스가 필요없는 심 속성까지 가지며 펜 클래스를 상속하면, 메모리가 낭비되는 것은 물론 오버라이드하는 줄도 모르고 가상 심의 뾰족함을 바꾸는 함수를 선언할 수도 있습니다.  오버라이드 된 함수 시그니처에 virtual 키워드가 붙지 않았다면 문제가 생깁니다.
...
3. 이번엔 오로지 코드 반복을 피하려고 상속을 썼습니다.  곧 1의 문제에 직면합니다.
4. 기존 코드를 건들지 않고 유지보수하려고 상속을 썼습니다.  1의 문제가 생기고 오히려 쓸모없는 코드가 증식했습니다.

**#씬*scene* 계층** **#추상화** **#위임*delegation***
<img src="https://user-images.githubusercontent.com/43042411/76703130-86641680-6712-11ea-82f0-3ba37f74c280.png"></img>
클래스 ::scene::inPlay::InPlay와 ::scene::online::Online은 인터페이스 ::scene::IScene의 구현이지만 씬이 아닙니다.  굳이 씬 매니저까지 나와 처리할 필요가 없는 것들을 추상화하기 위한 간이 씬 매니저입니다.  추상화는 코드를 읽기 쉽고, 분업하기도 쉽게 해줍니다.   
그리고 해제하지 않고 다음 씬으로 넘기고 싶은 인스턴스를 다루기 쉽습니다.   
( dlls/Game/scene/inPlay/Ready.h, Playing.h, GameOver.h의 생성자에 넘긴 sf::Drawable 레퍼런스 )   
또한, 클래스 ::scene::online::Online은 통신을 위한 전역변수에 접근하는 유일한 통로로서 하위 씬들에 위임됩니다.   
( dlls/Game/scene/online/ 아래 헤더 파일들 )

**#의존dependency 관계** **#Aggregation 관계**   
함수의 인수가 많지 않다면, 어떤 로컬 변수가 쓰이는 곳이 어딘지 쉽게 보이는 의존 관계를 선호합니다.   
그러나 
