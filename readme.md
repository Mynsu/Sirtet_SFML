...
**#루아*Lua* 스크립트**   
기획자와 아티스트가 부담없이 자주 수정할 수 있어야 하는 데이터와 리소스를 루아 스크립트로 다룹니다.   
로직은 빠르고 디버깅이 쉬워야 하므로 C++에서 다룹니다.

**#SFML**
SFML은 SDL처럼 오픈 소스 멀티미디어 라이브러리지만 레퍼런스가 적습니다.  그럼에도 SFML을 쓴 건 SDL과 달리 C++로 작성되었기 때문입니다.  OOP의 좋은 예를 뜯어볼 기회였고, 추상에서 구체로 학습할 수 있습니다.

**#콘솔**
[![Video Thumbnail](https://user-images.githubusercontent.com/43042411/76969660-0508c000-696e-11ea-897a-5b9b868422ae.jpg)](https://youtu.be/uPx0DgMIeoQ)
명령어와 함수를 매핑하여 런타임 중 콘솔로 여러 시도를 해볼 수 있습니다.  현재는 스크립트를 새로고침하거나, 원하는 씬으로 이동하거나, GUI가 미완성일 때를 대비해 방을 생성하거나 들어가는 명령어들이 등록되어 있습니다.   
명령을 비동기로 수행하므로 공유 데이터 관리가 필요합니다.   
( *exes/Engine/Console.h* )

**#로그**   
20년 3월 15일 현재, 파일로는 출력하지 않고 콘솔에만 출력합니다.
...
클라이언트가 접속할 때 악수와 함께 메시지를 보내더라도 서버가 받지 않습니다.  서버가 메시지를 기다리는 문제를 차단하기 위해서입니다.   
...
4. 클라이언트가 대기열 서버에 약속한 해시 값을 보냅니다.  대기열 서버는 해시값, 즉 초대장을 보고 가짜 클라이언트는 아닌지, 서버들과 버전이 같은지를 확인합니다.
5. 클라이언트가 주어진 시간 내에 4를 통과하고 메인 서버에도 자리가 있으면, 대기열 서버가 클라이언트와 메인 서버에 똑같은 티켓을 보냅니다.   
5-1-1. 클라이언트가 주어진 시간 내에 4를 통과했으나 메인 서버에 자리가 없으면, 대기열 서버는 클라이언트에 대기 번호를 발급하고 메인 서버에 자리가 났는지 주기적으로 물어봅니다.   
5-1-2. 메인 서버에 자리가 나면, 대기 번호 0 클라이언트에게 5를 수행합니다.   
5-2. 클라이언트가 주어진 시간 내에 4를 통과하지 못하면, 대기열 서버가 클라이언트의 접속을 끊고 시퀀스를 종료합니다.
...
**#TCP/IP**   
UDP는 패킷이 유실될 가능성이 조금이지만 있어 쓰지 않았습니다.
...
**#버퍼 복사하지 않기**   
대기열 서버가 accept 완료한 소켓들의 O/S 버퍼를 없애 복사하지 않습니다.  버퍼가 필요할 만큼 무거운 작업을 하지 않기 때문입니다.  대신 클라이언트 측 대기열 서버 소켓의 O/S 버퍼는 없애지 않았습니다.

**#백로그** **#::AcceptEx(...)**   
백로그만큼 ::AcceptEx(...)를 걸어두었습니다.

**#접속** **#Conditional Accept**   
소켓 옵션으로 conditional accept를 주어 서버가 accept를 완료해야 클라이언트가 접속할 수 있습니다.  이렇게 클라이언트가 접속 시도만으로 서버에 부하를 주지 못하도록 합니다.   

**#소켓 재활용** **#DisconnectEx**   
**#인스턴스 재활용**에서 설명한 것처럼 O/S로부터 소켓 핸들을 얻는 것은 비용이 큽니다.  그래서 접속을 끊은 소켓도 재활용할 수 있도록 ::DisconnectEx(...)를 씁니다.  보내기, 받기를 걸어두다 실패한 소켓만 새것으로 교체합니다.  이 소켓들은 ::DisconnectEx(...)도 실패하므로 재활용할 수 없기 때문입니다.  새것으로 교체할 때는 주의할 것이 있습니다.  교체하기 전에 걸어두었던 작업이 교체한 후에 새것의 키로 이벤트를 알립니다.  이것을 무시하는 처리를 해주어야 합니다.

**#스트레스 테스트**   
20년 3월 15일 현재, 클라이언트 100개까지 통과합니다.   

**#클라이언트** **#블로킹*Blocking* 소켓** **#멀티스레드**   
클라이언트에는 블로킹 소켓을 썼습니다.  연습으로 멀티스레드를 다루고 싶었기 때문입니다.   
정확히는 받는 것만 블로킹이고 보내는 것은 Overlapped I/O입니다.  초기에는 보내는 것도 블로킹으로 처리했습니다.  컨텍스트 스위칭과 시작 타이밍 때문에 보내거나 받을 때마다 스레드를 만들 순 없었습니다.  그래서 코드가 복잡했고 오작동도 잦았습니다.   
시간이 지나면 블로킹이 풀리도록 설정해 스레드를 join한 다음 소켓을 닫습니다.  간혹 오작동하기 때문에 강제로 소켓을 닫아 블로킹을 풀지 않습니다.

**#가짜 클라이언트 감지** **#해시 salt** **#키 스트레칭**   
**#대기열 서버**에서 '초대장'이 언급됩니다.  초대장은 복호화할 필요 없이 현재 시간과 salt와 키 스트레칭으로 얻은 해시 값이 적힙니다.

**#클라이언트와 서버가 공유하는 모델 애셋*asset***   
서버를 만들기 시작했을 때 클라이언트가 쓰던 모델 애셋을 그대로 가져와 썼었습니다.  곧, 렌더링에 쓰이는 멤버는 서버에게 불필요하다는 걸 알아챘습니다.  그래서 서버의 부하를 덜기 위해 필요없는 부위를 뺀 서버용 모델 애셋을 만들었습니다.   
서버용 모델 애셋을 따로 만들더라도 개발 막바지에 했어야 했습니다.  따로따로 수정하다보니 비효율적이고, 같아야 할 코드를 다르게 수정하는 등 실수가 나왔기 때문입니다.

**#디버그 모드에서 릴리즈 모드로** **#프로파일링**   
디버그 모드에서 릴리즈 모드로 바꾸고 난 뒤 이슈가 하나 있었습니다.  클라이언트가 메인 서버와 통신하다가 끊기는 이슈였습니다.  원인을 찾기 위해 프로젝트 설정을 소거해나갔습니다.  결국 메인 서버의 CRT가 /MDd가 아니라서 생긴 이슈임을 알아냈습니다.  릴리즈 모드의 기본값이 /MD이므로 원인은 DLL 여부가 아니라 디버그 여부라고 판단, 메인 서버를 /MTd로 빌드했습니다.  그런데 아뿔싸!  이젠 클라이언트를 /MDd로 빌드했을 땐 통신이 잘 되고 /MD로 빌드했을 땐 안 되는 것입니다.   
처음으로 돌아왔습니다.  '메인 서버가 느리다'는 겉보기부터 출발했습니다.  아니나다를까 릴리즈 모드로 빌드한 메인 서버가 한 방에 둘 이상이 들어오자마자 프레임 당 300 밀리초를 넘기는 것이었습니다.  프로파일링으로 WSAOVERLAPPED 구조체를 지우는 함수에 병목이 걸린다는 것을 알아냈습니다.  그리고 병목이 걸리는 이유는 의도와 달리 게임 루프마다 이 구조체가 만들어지기 때문이었습니다.  에러였습니다.

**#메소드 체이닝**   
입출력 스트림, 문자열 스트림 외엔 쓰지 않습니다.

**#템플릿 특수화** **#유니버설 레퍼런스** **#퍼펙트 포워딩**   
쓸 기회가 없었습니다.
...
**#클라이언트와 서버가 공유하는 모델 애셋**에서 언급한 것말고도 얻은 경험이 있습니다.  예를 들면, 로컬 게임과 달리 온라인 게임에는 입출력과 게임 로직 사이에 있는 시간 차이를 다루어야 합니다.  또, 온라인 게임은 로컬 게임을 기반, 입력부, 로직부, 출력(렌더)부로 나누어 로직부만 서버로 옮긴 것이라 여길 수도 있겠습니다.
